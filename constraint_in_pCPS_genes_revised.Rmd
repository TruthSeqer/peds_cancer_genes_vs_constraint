...
ADXLR_LoF_vs_all_ttest$p.value
...
ggsave("C:/Users/usto0005/Desktop/final_revised_figure_2_million.svg", plot = Figure_2_revised, height = 10, width = 10)
```
...
# loading precomputed downsampling [publicly available here: https://storage.googleapis.com/gcp-public-data--gnomad/release/2.1.1/constraint/gnomad.v2.1.1.lof_metrics.downsamplings.txt.bgz]
...
LoFs_in_tested %>% select(HGVS_for_distinct) %>%
  bind_rows(tibble(HGVS_for_distinct = as.character(rep(NA, n_tested_for_gene-n_LoFs_in_tested))))
...
df <- tibble()
#for (i in ADXLR_isolatedcancerrisk_names) {
  for (i in ADXLR_isolatedcancerrisk_names) {
  supp_plot <- downsampling_plot(input_gene = i)
  ggsave(paste0("L:/LovbeskyttetMapper/STAGING loggede data/CNS tumorer/constrained_sandbox/supp_plot_ds_",i,".svg"), plot = supp_plot, height = 1.5, width = 5)
  supp_source_data <- as.tibble(fig4c_func(supp_plot)) %>% mutate(gene = i)
  df <- bind_rows(df,supp_source_data)
  }

df %>% clipr::write_clip()
```

```{r supplementary_downsampling_plots}
for (i in "PAX5") {
  supp_plot <- downsampling_plot(input_gene = i)
  ggsave(paste0("L:/LovbeskyttetMapper/STAGING loggede data/CNS tumorer/constrained_sandbox/supp_plot_ds_",i,".svg"), plot = supp_plot, height = 1.5, width = 5)
  supp_source_data <- as.data.frame(fig4c_func(supp_plot))
  write_e(file = paste0("L:/LovbeskyttetMapper/STAGING loggede data/CNS tumorer/constrained_sandbox/supp_plot_ds_",i,".csv"), x = supp_source_data)
}

plot_for_export_SMARCA4 <- downsampling_plot(input_gene = "SMARCA4") %>% print()
plot_for_export_RB1 <- downsampling_plot(input_gene = "RB1") %>% print()
plot_for_export_APC <- downsampling_plot(input_gene = "APC") %>% print()
plot_for_export_DICER1 <- downsampling_plot(input_gene = "DICER1") %>% print()
plot_for_export_SMARCB1 <- downsampling_plot(input_gene = "SMARCB1") %>% print()
plot_for_export_TP53 <- downsampling_plot(input_gene = "TP53") %>% print()
```
...
# plotting ADXLR genes and level of missense constriant
mis_pCPSall_plot <- 
peds_CPS_constaint_anno %>% 
  filter(`ADDED: LoF vs GoF` %in% c("AD (LoF)", "XLR")) %>% 
  mutate(lof_constrained_evidence = case_when(
...
# plotting all ADXLR genes by mode of inheritance and level of missense constriant
mis_pCPSmode_plot <- 
peds_CPS_constaint_anno %>% 
  filter(`ADDED: LoF vs GoF` %in% c("AD (LoF)", "XLR")) %>% 
  mutate(lof_constrained_evidence = case_when(
...
# Supplementary & alternate plotting considered, notes, and scraps
...
As per journal guidelines, source data for each included plot was extracted in the following fashion (requires running code above!):

```{r}
#fig2a
ggplot_build(overview_randomset_plot)$data[[1]] %>%
  select(label,x,y,ymin,ymax,colour) %>% clipr::write_clip()

ggplot_build(overview_randomset_plot)$data[[2]] %>%
  select(yintercept,colour,linetype) %>% clipr::write_clip()

ggplot_build(overview_randomset_plot)$data[[3]] %>%
  select(label,x,y,colour,linetype) %>% clipr::write_clip()

ggplot_build(overview_randomset_plot)$data[[4]] %>%
  select(label,x,y,colour,linetype) %>% clipr::write_clip()

#fig2b
ggplot_build(overview_pCPSall_plot)$data[[1]] %>%
  select(label,x,y,ymin,ymax,colour) %>% clipr::write_clip()

ggplot_build(overview_pCPSall_plot)$data[[2]] %>%
  select(yintercept,colour,linetype) %>% clipr::write_clip()

ggplot_build(overview_pCPSall_plot)$data[[3]] %>%
  select(label,x,y,colour,linetype) %>% clipr::write_clip()

ggplot_build(overview_pCPSall_plot)$data[[4]] %>%
  select(label,x,y,colour,linetype) %>% clipr::write_clip()

#fig2c
ggplot_build(overview_pCPSmode_plot)$data[[1]] %>%
  select(label,PANEL,x,y,ymin,ymax,colour) %>% clipr::write_clip()

ggplot_build(overview_pCPSmode_plot)$data[[2]] %>%
  select(yintercept,colour,linetype) %>% clipr::write_clip()

ggplot_build(overview_pCPSmode_plot)$data[[3]] %>%
  select(label,PANEL,x,y,colour,linetype) %>% clipr::write_clip()

ggplot_build(overview_pCPSmode_plot)$data[[4]] %>%
  select(label,PANEL,x,y,colour,linetype) %>% clipr::write_clip()

#fig2d
#stat test metrics
table(canonical_constraint_metrics$ADXLR_LoF_pCPS_gene)
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_LoF_pCPS_gene, var.equal=T, exact = TRUE, ) %>% .$estimate
wilcox.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_LoF_pCPS_gene, var.equal=T, exact = TRUE, ) %>% .$p.value

table(sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene)
t.test(sizematched_constraint_metrics_54_for_test$oe_lof_upper ~ sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene, var.equal=T ) %>% .$estimate
wilcox.test(sizematched_constraint_metrics_54_for_test$oe_lof_upper ~ sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene, var.equal=T ) %>% .$p.value

table(canonical_constraint_metrics$AR_pCPS_gene)
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$AR_pCPS_gene, var.equal=T ) %>% .$estimate
wilcox.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$AR_pCPS_gene, var.equal=T ) %>% .$p.value

table(canonical_constraint_metrics$ADXLR_aCPS_gene)
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_aCPS_gene, var.equal=T ) %>% .$estimate
wilcox.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_aCPS_gene, var.equal=T ) %>% .$p.value

table(canonical_constraint_metrics$AR_aCPS_gene)
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$AR_aCPS_gene, var.equal=T ) %>% .$estimate
wilcox.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$AR_aCPS_gene, var.equal=T ) %>% .$p.value

#histogram input
ggplot_build(million_plot_for_figure_2)$data[[1]] %>% 
  mutate(bin = row_number()) %>% 
  select(bin,xmin,xmax,count,y,fill) %>% clipr::write_clip()

ggplot_build(million_plot_for_figure_2)$data[[2]] %>% 
  mutate(bin = row_number()) %>% 
  select(bin,xmin,xmax,count,y,fill) %>% clipr::write_clip()
...
peds_CPS_constaint_anno %>% 
  ...
  filter(!is.na(lof_constrained_evidence)) %>% 
  select(gene,Inheritance_sankey, lof_constrained_evidence, `ADDED: non_cancer_phenotype`,`ADDED: LoF vs GoF`) %>% 
  clipr::write_clip()

#fig3b
ggplot_build(ADXLR_vs_bar_plot)$data[[1]] %>% 
  select(fill, PANEL, x, y) %>% clipr::write_clip()

#fig3c
ggplot_build(pCPS_ADXLR_year_of_discovery)$data[[1]] %>% 
  select(label, group, x, y) %>% clipr::write_clip()

#fig4a_b
ds_iso_data <- 
downsampled_gnomAD %>% filter(gene%in%c(ADXLR_isolatedcancerrisk_names,"IKBKAP"), pop=="global", downsampling==100000) %>% 
  mutate(gene = if_else(gene == "IKBKAP", "ELP1", gene)) %>% 
  mutate(exp_lof=round(exp_lof,0)) %>% 
  select(gene, downsampling, exp_lof, obs_lof)

left_join(
canonical_constraint_metrics %>% 
  filter(gene%in%c(ADXLR_isolatedcancerrisk_names)) %>% 
  select(gene, cds_length, oe_lof, oe_lof_lower, oe_lof_upper),
ds_iso_data,
by = "gene") %>% 
  mutate(bp_per_exp_lof_100K = round(cds_length/exp_lof,0)) %>% clipr::write_clip()

#fig4c
fig4c_func <- function(plot) {
df <- 
  bind_rows(
  ggplot_build(plot)$data[[2]],
  ggplot_build(plot)$data[[4]],
  ggplot_build(plot)$data[[6]]) %>% 
  select(colour,x,y) %>% 
  mutate(n = 2^x,
         lofs = 2^y) %>% 
  filter(!is.na(y),
...
fig4c_func(plot_for_export_TP53) %>% clipr::write_clip()

#fig5a
ggplot_build(ped_mut_freq_plot)$data[[1]] %>%
  select(x,ymin,ymax, fill) %>% 
  arrange(x) %>% clipr::write_clip()

ggplot_build(ped_mut_prop_plot)$data[[1]] %>%
  select(x,ymin,ymax, fill) %>% 
  arrange(x) %>% clipr::write_clip()

#fig5c
ggplot_build(mis_pCPSall_plot)$data[[1]] %>%
  select(label,x,y,ymin,ymax,colour) %>% clipr::write_clip()

ggplot_build(mis_pCPSall_plot)$data[[2]] %>%
  select(yintercept,colour,linetype) %>% clipr::write_clip()

ggplot_build(mis_pCPSall_plot)$data[[3]] %>%
  select(label,x,y,colour,linetype) %>% clipr::write_clip()

ggplot_build(mis_pCPSall_plot)$data[[4]] %>%
  select(label,x,y,colour,linetype) %>% clipr::write_clip()

#fig5_d
ggplot_build(mis_pCPSmode_plot)$data[[1]] %>%
  select(label,PANEL,x,y,ymin,ymax,colour) %>% clipr::write_clip()

ggplot_build(mis_pCPSmode_plot)$data[[2]] %>%
  select(yintercept,colour,linetype) %>% clipr::write_clip()

ggplot_build(mis_pCPSmode_plot)$data[[3]] %>%
  select(label,PANEL,x,y,colour,linetype) %>% clipr::write_clip()

ggplot_build(mis_pCPSmode_plot)$data[[4]] %>%
  select(label,PANEL,x,y,colour,linetype) %>% clipr::write_clip()

#fig5e
#stat test metrics
table(canonical_constraint_metrics$ADXLR_LoF_pCPS_gene)
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$ADXLR_LoF_pCPS_gene, var.equal=T, exact = TRUE, ) %>% .$estimate
wilcox.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$ADXLR_LoF_pCPS_gene, var.equal=T, exact = TRUE, ) %>% .$p.value

table(sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene)
t.test(sizematched_constraint_metrics_54_for_test$oe_mis_upper ~ sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene, var.equal=T ) %>% .$estimate
wilcox.test(sizematched_constraint_metrics_54_for_test$oe_mis_upper ~ sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene, var.equal=T ) %>% .$p.value

table(canonical_constraint_metrics$AR_pCPS_gene)
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$AR_pCPS_gene, var.equal=T ) %>% .$estimate
wilcox.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$AR_pCPS_gene, var.equal=T ) %>% .$p.value

table(canonical_constraint_metrics$ADXLR_aCPS_gene)
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$ADXLR_aCPS_gene, var.equal=T ) %>% .$estimate
wilcox.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$ADXLR_aCPS_gene, var.equal=T ) %>% .$p.value

table(canonical_constraint_metrics$AR_aCPS_gene)
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$AR_aCPS_gene, var.equal=T ) %>% .$estimate
wilcox.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$AR_aCPS_gene, var.equal=T ) %>% .$p.value

#histogram input
ggplot_build(million_plot_for_figure_5)$data[[1]] %>% 
  mutate(bin = row_number()) %>% 
  select(bin,xmin,xmax,count,y,fill) %>% clipr::write_clip()

ggplot_build(million_plot_for_figure_5)$data[[2]] %>% 
  mutate(bin = row_number()) %>% 
  select(bin,xmin,xmax,count,y,fill) %>% clipr::write_clip()

#fig7
canonical_constraint_metrics %>% 
  filter(gene%in%c("CHEK2","PMS2","PALB2","BRCA1","ATM","BRCA2","MLH1","MSH6","MSH2","DIS3L2")) %>% 
  select(gene, oe_lof, oe_lof_lower, oe_lof_upper)%>% clipr::write_clip()
  
#supp fig 1
ggplot_build(cons_in_adult_vs_peds_mut_genes)$data[[1]] %>% 
  select(fill,PANEL,x,y) %>% clipr::write_clip()

#supp fig 2
ggplot_build(GWAS)$data[[1]] %>% 
  select(x, PANEL, y, violinwidth) %>% clipr::write_clip()
