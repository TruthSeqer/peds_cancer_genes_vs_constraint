...
adult_pan_cancer_data_PVs %>% 
  ...
  select(gene, oe_lof, oe_lof_upper, oe_lof_lower) %>% 
  mutate(lof_constrained_evidence = case_when(
...
pan_cancer_data[["Byrjalsen_pCPS_genes_2021"]] %>% 
  left_join(., canonical_constraint_metrics, by = "gene") %>% 
  select(gene, approx_de_novo_from_PMID_35820085, oe_lof_upper, oe_lof, `ADDED: de novo rate group`, `ADDED: Penetrance group`, Inheritance, `ADDED: non_cancer_phenotype`, `ADDED: LoF vs GoF`) %>% 
  filter(`ADDED: LoF vs GoF` %in% c("AD (LoF)","XLR")) %>%
...
#overall_ttest <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$pCPS_gene, var.equal=T )

# compares LOE scores for all 85 pCPS genes vs. all genes
overall_ttest_loe <- 
t.test(canonical_constraint_metrics$oe_lof ~ canonical_constraint_metrics$pCPS_gene, var.equal=T )

# compares LOEUF scores for monoallelic [AD/XLR] pCPS genes vs. all genes
ADXLR_vs_all_ttest <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_pCPS_gene, var.equal=T )

# compares LOEUF scores for monoallelic [AD/XLR], LoF pCPS genes vs. all other genes
ADXLR_LoF_vs_all_ttest <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_LoF_pCPS_gene, var.equal=T )

# compares LOEUF scores for monoallelic [AD], GoF pCPS genes vs. all other genes
#ADXLR_LoF_vs_all_ttest <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_GoF_pCPS_gene, var.equal=T )

# compares LOEUF scores for monoallelic [AD/XLR] pCPS genes vs. biallelic [AR] pCPS genes
ADXLR_vs_AR_ttest <- 
t.test(peds_CPS_constaint_anno$oe_lof_upper ~ peds_CPS_constaint_anno$ADXLR_pCPS_gene, var.equal=T )

# compares LOE scores for monoallelic [AD/XLR] pCPS genes vs. biallelic [AR] pCPS genes
ADXLR_vs_AR_ttest_loe <- 
t.test(peds_CPS_constaint_anno$oe_lof ~ peds_CPS_constaint_anno$ADXLR_pCPS_gene, var.equal=T )

# compares LOEUF scores for biallelic [AR] pCPS genes vs. all genes
AR_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$AR_pCPS_gene, var.equal=T )
...
t.test(peds_CPS_constaint_anno_ADXLR$oe_lof_upper ~ peds_CPS_constaint_anno_ADXLR$pCPS_isolatedcancerrisk_gene, var.equal=T )

# compares LOEUF scores for monoallelic [AD/XLR] pCPS genes with isolated cancer risk vs. all genes
#isoADXLR_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$pCPS_isolatedcancerrisk_gene, var.equal=T )

# compares MOEUF scores for monoallelic [AD/XLR] pCPS genes vs. all genes
mis_ADXLR_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$pCPS_ADXLR_gene, var.equal=T )
...
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$pCPS_notcon_gene, var.equal=T )

## aCPS GENES

# compares LOEUF scores of any gene pathogenically pLoF mutated in adult cancer patients vs. pediatric cancer patients
#adult_vs_peds_ttest <- 
t.test(adult_vs_peds_combined$oe_lof_upper ~ adult_vs_peds_combined$age, var.equal=T )

# compares LOEUF scores for all 75 pCPS genes vs. all genes
aCPS_overall_ttest <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$aCPS_gene, var.equal=T )

# compares LOE scores for all 75 aCPS genes vs. all genes
aCPS_overall_ttest_loe <- 
t.test(canonical_constraint_metrics$oe_lof ~ canonical_constraint_metrics$aCPS_gene, var.equal=T )

# compares LOEUF scores for monoallelic [AD/XLR] aCPS genes vs. all genes
aCPS_ADXLR_vs_all_ttest <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_aCPS_gene, var.equal=T )

# compares LOEUF scores for biallelic [AR] aCPS genes vs. all genes
aCPS_AR_vs_all_ttest <- 
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$AR_aCPS_gene, var.equal=T )

## ALL MISSENSE CONSTRAINT TESTS

# compares MOEUF scores for all pCPS genes vs. all genes
mis_pCPS_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$pCPS_gene, var.equal=T )

# compares MOEUF scores for monoallelic & LoF [AD/XLR] pCPS genes vs. all genes
#mis_ADXLR_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$ADXLR_LoF_pCPS_gene, var.equal=T )

# compares MOEUF scores for biallelic [AR] pCPS genes vs. all genes
mis_AR_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$AR_pCPS_gene, var.equal=T )

# compares MOEUF scores for monoallelic aCPS genes vs. all genes
aCPS_mis_aCPS_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$aCPS_gene, var.equal=T )

# compares MOEUF scores for monoallelic [AD/XLR] aCPS genes vs. all genes
aCPS_mis_ADXLR_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$ADXLR_aCPS_gene, var.equal=T )

# compares MOEUF scores for biallelic [AR] aCPS genes vs. all genes
aCPS_mis_AR_vs_overall_test <- 
t.test(canonical_constraint_metrics$oe_mis_upper ~ canonical_constraint_metrics$AR_aCPS_gene, var.equal=T )

## DE NOVO AND PENETRANCE

# compares LOEUF scores of pCPS genes associated with (very) high de novo rate vs. those with (very) low de novo rates
#de_novo_pCPS_genes_high_vs_low <- 
t.test(de_novo_rates$oe_lof_upper ~ de_novo_rates$de_novo_binary, var.equal=T )
...
t.test(de_novo_rates$oe_lof_upper ~ de_novo_rates$pen_binary, var.equal=T )

#comparisons of 53 AD(LoF)/XLR
t.test(canonical_constraint_metrics$oe_lof_upper ~ canonical_constraint_metrics$ADXLR_LoF_pCPS_gene, var.equal=T )

t.test(canonical_constraint_metrics$cds_length ~ canonical_constraint_metrics$ADXLR_LoF_pCPS_gene, var.equal=T )
...
t.test(sizematched_constraint_metrics_54_for_test$cds_length ~ sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene, var.equal=T )

t.test(sizematched_constraint_metrics_54_for_test$oe_lof_upper ~ sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene, var.equal=T )

t.test(sizematched_constraint_metrics_54_for_test$oe_mis_upper ~ sizematched_constraint_metrics_54_for_test$AD_LoF_XLR_gene, var.equal=T )

t.test(sizematched_constraint_metrics_54_for_test$oe_lof_upper ~ sizematched_constraint_metrics_54_for_test$pCPS_isolatedcancerrisk_gene, var.equal=T )
...
```{r gnomad_obs_function}
downsample_observed <- function(input_gene) {
LoF_counts <- 
gnomad_LoF %>% 
  mutate(gene = if_else(gene == "IKBKAP", "ELP1", gene)) %>% 
  filter(gene == input_gene,
         AC > 0) %>% 
  mutate(genotype_number = row_number()) %>% 
  select(genotype_number,AC)

print(paste0("gene: ", input_gene))

df <- tibble(value = rep(0, 125748))

for (i in 1:nrow(LoF_counts)) {
  locus_count <- LoF_counts %>% filter(genotype_number == i) %>% pull(AC)
  indices <- sample(1:125748, locus_count)
  col_name <- paste0("genotype_", i)
  df <- df %>% mutate({{col_name}} := ifelse(row_number() %in% indices, 1, 0))
}
...
print(paste0("downsampling to n tested: ",n_tested_for_gene))
...
combined_pancancer_data_cleaned %>%  
  ...
  filter(gene == input_gene,
         ontology_cleaned %in% c("nonsense", "frameshift", "pathogenic_splice")) %>% 
  select(study_year, gene, c.HGVS, p.HGVS, ontology_cleaned) %>%
  print() %>%
  distinct(c.HGVS, p.HGVS, ontology_cleaned, .keep_all = T) %>% 
  print() %>%
  nrow()

downsample_stats <- map_dfr(1:100, function(i) {
  df %>%
    select(-value) %>%
    sample_n(n_tested_for_gene) %>%
    pivot_longer(cols = starts_with("genotype_"), names_to = "genotype", values_to = "value") %>%
    filter(value == 1) %>%
    summarise(gene = input_gene,
              distinct_loci = n_distinct(genotype),
              AC_sum = sum(value),
              run = i,
              pancan_obs = n_LoFs_in_tested)
})

print(paste0("observed LoF variants in the ",n_tested_for_gene, " children with cancer tested:"))
extrapolated_n_LoFs_in_tested <- round(n_LoFs_in_tested*sqrt(100000/n_tested_for_gene),0)
print(paste0("distinct gnomAD alleles: ", n_distinct(LoF_counts$genotype_number)))
print(paste0("total gnomAD alleles: ", sum(LoF_counts$AC)))
print(paste0("case n: ", n_tested_for_gene))
print(paste0("case n alleles: ", n_tested_for_gene*2))
print(paste0("case LoF variants: ", n_LoFs_in_tested))
print(paste0("case extrapolated LoF variants: ", extrapolated_n_LoFs_in_tested))

return(downsample_stats)
}
```

```{r obs_data_generation}
cum_downsample_data <- 
tibble(gene = character(),
       distinct_loci = integer(),
       AC_sum = integer(),
       run = integer(),
       pancan_obs = integer())
...
for (x in ADXLR_isolatedcancerrisk_names) { 
cum_downsample_data <- bind_rows(
  cum_downsample_data,
  downsample_observed(x))
}
```

```{r plot_obs_vs_downsampled_gnomad}
## Reordering cum_downsample_data$gene
cum_downsample_data$gene <- cum_downsample_data$gene %>%
  fct_relevel(
    "SMARCA4", "TRIM28", "RB1", "CTR9", "APC", "PAX5", "DICER1",
    "SMARCE1", "SMARCB1", "FBXW7", "TRIP13", "BAP1", "REST", "TP53",
    "GPR161", "ELP1", "CDKN2A"
  ) 

pancan_vs_gnomad_observed_plot <- 
cum_downsample_data %>% 
  ggplot(aes(x = distinct_loci)) +
  geom_bar(color = blue,
           fill = "white",
           size = 1) +
  #geom_bar(aes(x = pancan_obs), fill = blue) +
  geom_segment(aes(x=pancan_obs, y=25, xend=pancan_obs, yend=5),
               arrow = arrow(length=unit(.15, 'cm'),type = "closed"),
               size = 1,
               color = "#C03830", lineend = "square") +
...
ggsave("L:/LovbeskyttetMapper/STAGING loggede data/CNS tumorer/constrained_sandbox/Figure_4_obs.svg", plot = pancan_vs_gnomad_observed_plot, height = 20, width = 4)
```

```{r 100K_inference_function}
downsample_inferred <- function(input_gene) {
LoF_counts <- 
gnomad_LoF %>% 
  mutate(gene = if_else(gene == "IKBKAP", "ELP1", gene)) %>% 
  filter(gene == input_gene,
...
print(paste0("downsampling to 100,000"))
...
downsample_stats_100K <- map_dfr(1:100, function(i) {
  df %>%
    select(-value) %>%
    sample_n(100000) %>%
    pivot_longer(cols = starts_with("genotype_"), names_to = "genotype", values_to = "value") %>%
    ...
    summarise(gene = input_gene,
              ...
              AC_sum = sum(value),
              run = i,
              pancan_inf = round(n_LoFs_in_tested*sqrt(100000/n_tested_for_gene),0))
})
...
return(downsample_stats_100K)
}
```

```{r 100K_inference_data_generation}
cum_downsample_data_inf <- 
tibble(gene = character(),
       distinct_loci = integer(),
       AC_sum = integer(),
       run = integer(),
...
for (x in ADXLR_isolatedcancerrisk_names) { 
cum_downsample_data_inf <- bind_rows(
  cum_downsample_data_inf,
  downsample_inferred(x))
}
```

```{r plot_inf_vs_100K_gnomad}
## Reordering cum_downsample_data$gene
cum_downsample_data_inf$gene <- cum_downsample_data_inf$gene %>%
  fct_relevel(
    "SMARCA4", "TRIM28", "RB1", "CTR9", "APC", "PAX5", "DICER1",
    "SMARCE1", "SMARCB1", "FBXW7", "TRIP13", "BAP1", "REST", "TP53",
    "GPR161", "ELP1", "CDKN2A"
...
pancan_vs_gnomad_inferred_plot <- 
cum_downsample_data_inf %>% 
  ggplot(aes(x = distinct_loci)) +
  geom_bar(color = blue,
           fill = "white",
           size = 1) +
  #geom_bar(aes(x = pancan_obs), fill = blue) +
  geom_segment(aes(x=pancan_inf, y=25, xend=pancan_inf, yend=5),
               arrow = arrow(length=unit(.15, 'cm'),type = "closed"),
               size = 1,
               color = "#C03830", lineend = "square") +
...
ggsave("L:/LovbeskyttetMapper/STAGING loggede data/CNS tumorer/constrained_sandbox/Figure_4_inf.svg", plot = pancan_vs_gnomad_inferred_plot, height = 20, width = 4)
```
...
# perform a t-test to compare the constraint scores of the genes with risk SNPs and the genes without risk SNPs
t.test(GWAS_meta_filtered$oe_lof_upper ~ GWAS_meta_filtered$Adult_vs_children, var.equal=T )
...
t.test(GWAS_anno_cons$oe_lof_upper ~ GWAS_anno_cons$risk_SNP, var.equal=T )
```
